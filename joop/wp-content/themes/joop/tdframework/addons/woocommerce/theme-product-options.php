...
global $theme_product_options;

// Register products option sections
// Products option for themes
//

$theme_product_options = new CoreOptionHandler(THEME_SLUG . '-product-options', THEME_NAME . ' options', array('product'));
core_options_handler_register($theme_product_options);

$group = new CoreOptionGroup('general', __('General', THEME_SLUG));
$theme_product_options->group_add($group);
...
//Background Set-up
$option = new CoreOption('background_setup', __('Page Background', THEME_SLUG), 'select', __('You can have a background image or the featured image as the background.', THEME_SLUG), 'none','bg-options');
$option->parameters = array('none' => 'none', 'image' => 'Image', 'featured' => 'Featured Image');
