...
foreach ( $items as $item ) :
	...
	<tr>
		<td style="text-align:left; vertical-align:middle; border: 1px solid #eee; word-wrap:break-word;"><?php
			...
			if ( $show_image ) {
				echo apply_filters( 'woocommerce_order_item_thumbnail', '<img src="' . current( wp_get_attachment_image_src( get_post_thumbnail_id( $_product->id ), 'thumbnail') ) .'" alt="' . __( 'Product Image', THEME_SLUG ) . '" height="' . esc_attr( $image_size[1] ) . '" width="' . esc_attr( $image_size[0] ) . '" style="vertical-align:middle; margin-right: 10px;" />', $item );
			}
			...
			if ( $show_download_links && $_product->exists() && $_product->is_downloadable() ) {
				...
				foreach ( $download_files as $download_id => $file ) {
					...
					if ( count( $download_files ) > 1 ) {
						$prefix = sprintf( __( 'Download %d', THEME_SLUG ), $i );
					} elseif ( $i == 1 ) {
						$prefix = __( 'Download', THEME_SLUG );
					}
