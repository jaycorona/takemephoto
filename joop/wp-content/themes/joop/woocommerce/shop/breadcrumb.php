...
if ( ( ! is_home() && ! is_front_page() && ! ( is_post_type_archive() && get_option( 'page_on_front' ) == woocommerce_get_page_id( 'shop' ) ) ) || is_paged() ) {
	...
	if ( is_category() ) {
		...
		echo $prepend . $before . __( 'Products tagged &ldquo;', THEME_SLUG ) . $queried_object->name . '&rdquo;' . $after;
		...
		if ( is_search() ) {
			...
			echo $before . '<a href="' . get_post_type_archive_link('product') . '">' . $_name . '</a>' . $delimiter . __( 'Search results for &ldquo;', THEME_SLUG ) . get_search_query() . '&rdquo;' . $after;
		...
		echo $before . __( 'Error 404', THEME_SLUG ) . $after;
		...
		echo $before . __( 'Search results for &ldquo;', THEME_SLUG ) . get_search_query() . '&rdquo;' . $after;

	} elseif ( is_tag() ) {

			echo $before . __( 'Posts tagged &ldquo;', THEME_SLUG ) . single_tag_title('', false) . '&rdquo;' . $after;

	} elseif ( is_author() ) {

		$userdata = get_userdata($author);
		echo $before . __( 'Author:', THEME_SLUG ) . ' ' . $userdata->display_name . $after;
